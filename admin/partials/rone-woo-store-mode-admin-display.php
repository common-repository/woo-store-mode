 ...
 * @subpackage rone_woo_store_mode/admin/partials
 */
// echo "<pre>";
// var_dump( $options );
// var_dump($modes);
...
<div class="wrap">
	...
	<p><strong>Mode:</strong> <?php echo ucfirst( rone_get_current_mode() ); ?>, <strong>Status:</strong> <?php echo rone_store_status(); ?></p>
	<p><strong>Store Time:</strong> <?php echo date('H:i', current_time('timestamp') ); ?></p>
	<h2>Enable/Disable Modes</h2>
	<form id="rone-woo-store-options" method="post">
		<?php if( $modes ): ?>
		...
		<?php 
			...
			$checkout_content = isset( $options['Rone_Woo_store_checkout_content'] ) ? $options['Rone_Woo_store_checkout_content'] : '';
			
			wp_editor( $checkout_content, 'Rone_Woo_store_checkout_content' );

			wp_nonce_field( 'Rone_Woo_store_nonce', '_wpnonce', false, true );

		?>

		<input class="button button-primary" type="submit" name="save_Rone_Woo_store_options" value="Save">
...
<script type="text/javascript">
	/* <![CDATA[ */
	var rone_store_options = <?php echo json_encode( rone_get_options() ); ?>;
		...
		<table class="wp-list-table widefat">
			...
			<tbody>
				<# _.each( mode_terms, function( term, termIndex ) { 
					...
					var openTime = '00:00';
					var closeTime = '00:00';
					if( typeof rone_store_options['store'][row] != 'undefined' ) {
						openTime = rone_store_options.store[row][termSlug].open;
						closeTime = rone_store_options.store[row][termSlug].close;
					}
					console.log('testing');
				#>

				<!-- if( rone_store_options.store[row][termSlug] ) {
						openTime = rone_store_options.store[row][termSlug].open;
						closeTime = rone_store_options.store[row][termSlug].close;
					} -->
				<tr>
